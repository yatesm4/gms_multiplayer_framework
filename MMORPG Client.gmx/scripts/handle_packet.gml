...
switch(command){
    case "HELLO":
        ...
        room_goto_next();
        show_debug_message("Server Arrival: " + server_time);
        break;
    ...
    case "LOGIN":
        ...
        if(status == "TRUE"){
            ...
            //Initiate a player object into room
            with(instance_create(target_x, target_y, obj_Player)){
                name = other.name;
            }
            
        }else{
            show_message("Login Failed: User Doesn't Exist || Password Incorrect");
        }
        break;
    ...
    case "REGISTER":
        status = buffer_read(argument0, buffer_string);
        if(status == "TRUE"){
            show_message("Register Successful. You may now login.");
        }else{
